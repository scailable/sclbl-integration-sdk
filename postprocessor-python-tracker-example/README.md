...
This example application provides an example on how to implement an external tracking system and integrate with Nx Objects. The postprocessor can add UUID's to the return object, which would then be used to identify the objects in the Nx server. Not all classes have to be given ID's, but all objects within a class need ID's, otherwise all ID's for the class will be ignored and the default tracking system in the NXAI Plugin will be used.
...
``` json
{
    "externalPostprocessors": [
        {
            "Name":"Example-Tracker-Postprocessor",
            "Command":"/opt/networkoptix-metavms/mediaserver/bin/plugins/nxai_plugin/nxai_manager/postprocessors/postprocessor-python-tracker-example",
